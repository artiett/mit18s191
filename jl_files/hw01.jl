...
function vecvec_to_matrix(vec_of_vecs)
...
function mean_colors(image)
	return (mean([color.r for color in image]),
		mean([color.g for color in image]),
		mean([color.b for color in image]))
end
...
# ╔═╡ 08cd5a4e-1db8-11eb-17ea-53a7be03dd1e
RGBX(0.199, 0.299, 0.6967), quantize(RGBX(0.199, 0.299, 0.6967))

# ╔═╡ f431d686-1db8-11eb-0f33-b12b88a337e6
...
# ╔═╡ c4a91a4a-1dc3-11eb-13d1-c5cd0eb22e86
begin
	function myclamp(x, lo, hi)
		return x < lo ? x = max(x, lo) : x = min(x, hi)
	end
end
...
begin
	function noisify(x::Number, s)
		seed = rand()
		noise = rand((seed, -seed))
		return ((noise ≤ s) & (noise ≥ -s)) ? x += noise : x += myclamp(noise, -s, s)
	end
...
# ╔═╡ ea41749e-1dca-11eb-10af-99b86a2ac080
noisify(10.0, 0.95)
...
begin
	...
	function extend(v, arr::AbstractArray)
		return extend.(Ref(v), arr)
	end
end
...
# ╔═╡ 416d9e02-1ddd-11eb-19be-f3fb6a2aec58
reverse(v)
...
function blur_1D(v, l)
	return [mean(extend(v, i-l:i+l)) for i in eachindex(v)]
end
...
function convolve_vector(v, k)
	ℓ = (length(k) - 1) ÷ 2
	copyofv = similar(v)
	copyofv = [sum(extend(v, i-ℓ:i+ℓ).*reverse(k)) for i in 1:length(v)]
	return copyofv
end
...
colored_line(test_convolution) # ну и чего?
...
@bind gkernel_size Slider(1:15, show_value=true)

# ╔═╡ c5e54094-2400-11eb-35c5-99d52ec3a0a9
gaussian_kernel(gkernel_size)

# ╔═╡ 38eb92f6-ee13-11ea-14d7-a503ac04302e
test_gauss_1D_a = let
	v = random_vect
	k = gaussian_kernel(gkernel_size)
...
begin
	function extend_mat(M::AbstractMatrix, i, j)
		...
		I,J = myclamp(i, 1, num_rows), myclamp(j, 1, num_columns)
		return M[I,J]
	end
end
...
function convolve_image(M::AbstractMatrix, K::AbstractMatrix)
	...
	mrows, mcols = size(M)
	κ, λ = (size(K) .- 1) .÷ 2
	copyofm = fill(convert(typeof(M[1,1]),0), mrows, mcols)
	for i in 1:mrows
		for j in 1:mcols
			for k in 1:krows
				for l in 1:kcols
					copyofm[i,j] += extend_mat(M, i-κ+k-1, j-λ+l-1) * extend_mat(K, k, l)
				end
			end
		end
	end
...
# ╔═╡ d4f8acc4-2ca7-11eb-06a6-9b1eb8719f0d
@bind gkernel_size_2D Slider(1:15, show_value=true)
...
# ╔═╡ 24a7be38-2cab-11eb-1ead-5507d9ae519d
function show_colored_kernel(kernel)
	to_rgb(x) = RGB(max(-x, 0), max(x, 0), 0)
	to_rgb.(kernel) / maximum(abs.(kernel))
end

# ╔═╡ f1b82008-2caa-11eb-33f4-f5c947a7bb99
show_colored_kernel(Kernel.gaussian((gkernel_size_2D, gkernel_size_2D)))
