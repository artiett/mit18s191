...
unused_letters = let
	[alphabet[i] for (i, x) in enumerate(sample_freqs) if x==0]
end
...
# ╔═╡ f5917394-b26b-4ed2-9d44-5995b47abc98
begin
	function freq_of_pair(pair::String, matrix)
		if length(pair) != 2
			pair = SubString(pair, 1:2)
		end
		return matrix[index_of_letter(Char(pair[1])), index_of_letter(Char(pair[2]))]
	end
end
...
th_frequency = freq_of_pair("th", sample_freq_matrix)

# ╔═╡ 1f94e0a2-f9ab-11ea-1347-7dd906ebb09d
md"""👉 What about `"ht"`?"""

# ╔═╡ 41b2df7c-f931-11ea-112e-ede3b16f357a
ht_frequency = freq_of_pair("ht", sample_freq_matrix)
...
# ╔═╡ c266cb28-68c2-416f-8ceb-605f39d92c1b
begin
	function list_of_doubles(matrix::Matrix)
		doubles = []
		...
		return [alphabet[i] for i in doubles]
	end
	
	# have found a diag() function, which returns a diagonal of the matrix
	# (main by default);  it’s more Julian way, but does not saves running time
	# function list_of_doubles_2(matrix::Matrix)
		# return [alphabet[i] for i in findall(!isequal(0.0), diag(matrix))]
	# end
end

# ╔═╡ 65c92cac-f930-11ea-20b1-6b8f45b3f262
double_letters = list_of_doubles(sample_freq_matrix)

# ╔═╡ 73df599d-9cf7-4cd8-8ab7-49957277f0e3
begin
	
	function most_freq_follower(char::Char, matrix::Matrix)
		letter_combs = matrix[index_of_letter(char), 1:end]
		f = findmax(letter_combs)[2]
		return alphabet[f]
	end
	
	function most_freq_precursor(char::Char, matrix::Matrix)
		letter_combs = matrix[1:end, index_of_letter(char)]
		f = findmax(letter_combs)[2]
		return alphabet[f]
	end
	
# 	function sum_of_followers(matrix::Matrix)
# 		sums = []
# 		for row in matrix
# 			for j in row
# 			push!(sums, sum(matrix[row, :]))
# 		end
# 		return sums
# 	end
	
# 	function sum_of_precursors(matrix::Matrix)
# 		sums = []
# 		for col in 1:size(matrix)[2]
# 			push!(sums, sum(matrix[:, col]))
# 		end
# 		return sums
# 	end
...
most_likely_to_follow_w = most_freq_follower('w', sample_freq_matrix)
...
most_likely_to_precede_w = most_freq_precursor('w', sample_freq_matrix)
...
# ╔═╡ 802d2d48-8f1b-4e74-a8a9-2fccc597d994
begin
	a_list = []
	for row in sample_freq_matrix
		for j in row
		push!(sums, sum(matrix[row, :]))
	end
end

# ╔═╡ 01c00880-e43c-483b-9209-38bac457cc64
a_list
...
try
	...
	"""<h2>It looks like this text is <i>$(argmin(distances))</i>!</h2>""" |> HTML
catch
end
